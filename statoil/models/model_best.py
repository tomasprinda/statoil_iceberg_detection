...
class SimpleCNN(Model):
    ...
    def init_graph(self):
        ...
        def fcn_block(name, input_, out_size, relu, dropout):
            with tf.variable_scope(name):
        ...
        # Net
        activations = cnn_block(self.tf_x, self.conf["filter_size"], self.conf["cnn_1_out_channels"], "cnn_1", dropout=self.conf["cnn_1_dropout"], bn=False, stride=1)
        activations = cnn_block(activations, self.conf["filter_size"], self.conf["cnn_2_out_channels"], "cnn_2", dropout=self.conf["cnn_2_dropout"], bn=False, stride=1)
        activations = cnn_block(activations, self.conf["filter_size"], self.conf["cnn_3_out_channels"], "cnn_3", dropout=self.conf["cnn_3_dropout"], bn=False, stride=1)
        activations = cnn_block(activations, self.conf["filter_size"], self.conf["cnn_4_out_channels"], "cnn_4", dropout=self.conf["cnn_4_dropout"], bn=False, stride=1)
        ...
        activations = fcn_block("fcn_1", activations, out_size=512, dropout=True, relu=True)
        activations = fcn_block("fcn_2", activations, out_size=256, dropout=True, relu=True)
        logits = fcn_block("fcn_3", activations, out_size=cfg.NUM_LABELS, dropout=False, relu=False)
        ...
        with tf.variable_scope("loss"):
            # self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.tf_label, logits=logits))
